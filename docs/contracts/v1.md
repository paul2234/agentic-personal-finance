...
- `accounts list [--json]`
- `accounts create --code <code> --name <name> --type <type> --normal-side <side> [--allow-contra] [--json]`
- `accounts create --from-file <path> [--allow-contra] [--json]`
- `ledger post-entry --file <path> [--idempotency-key <key>] [--json]`
- `transactions import --file <path> [--json]`
...
Creates a chart-of-accounts record.
...
- If `accountType` and `normalSide` mismatch, request is rejected unless `allowContra: true`.
- Example: `ASSET` + `CREDIT` requires explicit contra confirmation.

### POST `/create-accounts-batch`

Creates multiple chart-of-accounts rows from one request.

Request body:

```json
{
  "allowContraByDefault": false,
  "accounts": [
    {
      "code": "1500",
      "name": "Prepaid Expense",
...
Success response includes per-batch counters and item-level errors.
...
Imports bank/card transactions for a single mapped account.
...
- `CONTRA_CONFIRMATION_REQUIRED` - contra account creation requires explicit confirmation
